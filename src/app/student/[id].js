...
import { View, Text, StyleSheet, Platform, Alert, Pressable, Share } from 'react-native';
import { useLocalSearchParams, useRouter } from 'expo-router';
import { Ionicons } from '@expo/vector-icons';
import {
  Screen, Card, Button, IconBtn, Sheet, Field, T, Avatar, ExamTag, ScoreJourney,
  Divider, EmptyState, SectionTitle, ChipSelect,
} from '../../components/ui';
import { AddStudentSheet } from '../(tabs)/students';
import { ShareSheet } from '../(tabs)/library';
import { useData, balanceFor, nextLessonFor } from '../../lib/store';
import { openFile } from '../../lib/files';
import { generateRoadmap } from '../../lib/roadmap';
import { generateCurriculum, UNIT_STATUS } from '../../lib/curriculum';
import { colors, fonts, examMeta } from '../../lib/theme';
import { uid, todayKey, addDays, formatShort, formatTime, formatLong } from '../../lib/dates';
...
export default function StudentProfile() {
  ...
  return (
    <Screen>
      ...
      <SectionTitle icon="key">Student account</SectionTitle>
      <AccountCard student={student} data={data} />
...
// ---------- account linking ----------
function AccountCard({ student, data }) {
  const linked = (data.profilesList || []).find((p) => p.student_id === student.id);
  const pending = (data.profilesList || []).filter(
    (p) => p.role === 'student' && !p.student_id,
  );
  ...
  return (
    <Card>
      {linked ? (
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          <Ionicons name="checkmark-circle" size={20} color={colors.pine} style={{ marginRight: 10 }} />
          <View style={{ flex: 1 }}>
            <T.semi>{linked.display_name || linked.email}</T.semi>
            <T.small>{linked.email} · has access to their portal</T.small>
          </View>
          <Button
            title="Unlink" kind="danger" small
            onPress={() => data.unlinkAccount(linked.user_id)}
          />
        </View>
      ) : (
        <>
          <T.muted style={{ marginBottom: pending.length ? 10 : 0 }}>
            {pending.length
              ? `No account linked yet. ${pending.length} signed-up account${pending.length === 1 ? '' : 's'} waiting to be linked:`
              : 'No account linked. Ask the student to sign up in the app, then their account will appear here to link.'}
          </T.muted>
          {pending.map((p, i) => (
            <View key={p.user_id} style={[{ flexDirection: 'row', alignItems: 'center', paddingVertical: 8 }, i > 0 && { borderTopWidth: 1, borderTopColor: colors.line }]}>
              <Ionicons name="person-circle-outline" size={22} color={colors.faint} style={{ marginRight: 10 }} />
              <View style={{ flex: 1 }}>
                <T.body>{p.display_name || p.email}</T.body>
                <T.small>{p.email}</T.small>
              </View>
              <Button
                title="Link to this student" kind="quiet" small
                onPress={() => data.linkStudentAccount(p.user_id, student.id)}
              />
            </View>
          ))}
        </>
      ...
      <T.small style={{ marginTop: 10 }}>
        Linked students can see their lessons, homework, roadmap, curriculum, and shared documents —
        never payments, rates, your notes, or other students.
      </T.small>
    </Card>
  );
}
